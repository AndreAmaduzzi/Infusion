...
class Uniform15KPC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
                 ...
                 input_dim=3, use_mask=False, get_text2shape_ds=False, 
                 root_text2shape="", t2s_lowercase=False, t2s_language_model=''):
        
        print('want Text2Shape?', get_text2shape_ds)
        self.root_dir = root_dir
        self.split = split
        self.in_tr_sample_size = tr_sample_size
        self.in_te_sample_size = te_sample_size
        self.subdirs = subdirs # contains the name of the folder of categories (example, "03001627")
        self.scale = scale
        ...
        self.all_key_pad_masks = []
        self.max_length_text = 60
        count_not_found = 0
        
        if get_text2shape_ds:
            # get text2shape ds
            if "03001627" in self.subdirs:
                if "04379243" in self.subdirs:
                    cate="all"
                else:
                    cate="chair"
            elif "04379243" in self.subdirs:
                cate='table'
            else:
                raise Exception('category name is wrong')
            print('subdirs: ', self.subdirs)
            print('category: ', cate)
            print('root_t2s: ', root_text2shape)
            annotations_path = os.path.join(root_text2shape, "annotations", "from_shapenet_v1", f"{self.split}_{cate}.csv")
            ...
            for idx, row in tqdm(df.iterrows()):
                ...
                found=False
                # search for the model id in ShapeNet ds => add pcd to list
                    ...
                    for split in ['train', 'val', 'test']:
                        ...
                        for model in os.listdir(sub_path):
                            if model_id in model:
                                ...
                                all_mids.append(model_id)
                                # prepare pcd
                                point_cloud = np.load(model_path)
                                try:
                                    point_cloud = np.load(model_path)  # (15k, 3)
                                except:
                                    continue
                                assert point_cloud.shape[0] == 15000
                            
                                # add other fields (text, text embed)
                                text = row["description"]
                                text = text.strip('"')
                                category = row["category"]
                                tensor_name = row["tensor"]
                                if t2s_lowercase:
                                    text_embed_path = os.path.join(root_text2shape, "text_embeds", t2s_language_model, "lowercase", f"{tensor_name}")
                                else:
                                ...
                                self.cate_idx_lst.append(idx)
                                self.all_cate_mids.append((subd, model_id))
                                self.all_texts.append(text)
                                self.all_text_embeds.append(text_embed)
                                self.all_key_pad_masks.append(key_padding_mask)

                if found==False:
                    count_not_found += 1
            print(count_not_found ,'shapes not found' )
        else:
            for cate_idx, subd in enumerate(self.subdirs):
...
class ShapeNet15kPointClouds(Uniform15KPC):
    def __init__(self, root_dir="data/ShapeNetCore.v2.PC15k",
                 ...
                 random_subsample=False,
                 all_points_mean=None, all_points_std=None,
                 use_mask=False, get_text2shape_ds=False,
                 root_text2shape="", t2s_lowercase=False,
                 t2s_language_model=""):
        self.root_dir = root_dir
        self.split = split
        ...
        super(ShapeNet15kPointClouds, self).__init__(
            ...
            input_dim=3, use_mask=use_mask, get_text2shape_ds=get_text2shape_ds,
            root_text2shape=root_text2shape, t2s_lowercase = t2s_lowercase, t2s_language_model=t2s_language_model)
